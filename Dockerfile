...
COPY --from=server-build --chown=nix:nix /build/package.yaml /build/tls /build/nix/deploy.nix /build/stack.yaml /build/README.md  /build/Setup.hs /build/LICENSE  /server/
